init 10 python:
    class TearDisplayable(object):
        ...
        """
        def __init__(self, number=10, times=(0.24, 0.24), offsetRange=(0, 50), chroma=True, key=None, render_child=True):
            self.number = number
            self.times = times
        ...
        def __call__(self, child):
            rv = _TearDisplayable(child, self.number, self.times, self.offsetRange, self.chroma, self.render_child)
    ...
    class TearSurface(_BaseTear):
        ...
        """
        def __init__(self, number=10, times=(0.24, 0.24), offsetRange=(0, 50), chroma=True, render_child=True, srf=None):
            super(TearSurface, self).__init__(number, times, offsetRange, chroma, render_child)
            self.srf = srf or renpy.display.draw.screenshot(None)
        ...
        def __getstate__(self):
            ...
            rv = super(TearSurface, self).__getstate__()
            self.srf = srf
        ...
        def __setstate__(self, state):
            super(TearSurface, self).__setstate__(state)   
            self.srf = renpy.display.pgrender.load_image(io.BytesIO(self.srf), "TearSurface.png")
    ...
    class GlicthedDisplayable(object):
        ...
        `rotate`: int | float
            A number to rotate the rectangle by.
        ...
        def add_glitch(self, center=(0.5, 0.5), cols=5, rows=5, size=(50, 50), box=(0.0, 0.0, 1.0, 1.0), times=(0.24, 0.24), d=None, rotate=0):
            self.glitches.append((center, max(cols, 1), max(rows, 1), size, box, times, d, rotate))
            return self
...
screen tear(*args, **kwargs):
    zorder 1000
    default surf = TearSurface(*args, **kwargs)
    add surf:
        at transform:
            xysize (1280, 720)
...
init python:
    class _TearPiece(object):
        def __init__(self, startY, endY, times, range):
            self.y = max(0, startY)
            self.height = max(0, endY - startY)

            self.onTime, self.offTime = [x * random.random() for x in times]
    ...
    class _TearCore(object):
        ...
        def __init__(self, number, times, offsetRange, chroma, render_child):
            ...
            self.pieces = [
                _TearPiece(tearpoints[i], tearpoints[i + 1], times, offsetRange)
    ...
    class _BaseTear(renpy.Displayable):
        def __init__(self, number, times, offsetRange, chroma, render_child):
            ...
            self.tear = _TearCore(number, times, offsetRange, chroma, render_child)
    ...
    class _TearDisplayable(_BaseTear):
        def __init__(self, child, number, times, offsetRange, chroma, render_child):
            super(_TearDisplayable, self).__init__(number, times, offsetRange, chroma, render_child)
...
init python:
    class _GlicthedDisplayablePiece(object):
        def __init__(self, col, row, times):
            self.col = col
            self.row = row
    ...
    class _GlicthedDisplayableCore(object):
        def __init__(self, center, cols, rows, size, box, times, d, rotate):
            self.pieces = [
                _GlicthedDisplayablePiece(col, row, times)
...
rect: ({}, {})""".format(xrange, yrange, rect_xsize, rect_ysize))
            ...
            rm = Matrix.rotate(0, 0, self.rotate)
            manchorx, manchory = sw / 2.0, sh / 2.0

            m = Matrix.offset(-manchorx, -manchory, 0.0)
            m = rm * m
            m = Matrix.offset(manchorx, manchory, 0.0) * m

            rect_render.reverse = renpy.display.render.IDENTITY * m
