...
python early in phone:
    ...
    class _RawInitPhoneRegister(_RawPhoneRegister):
        ...
        def _execute(self):
            ...
            if self.default_statement is not None:
                ...
                if self.name is not None:
                    name = eval(self.name, globals)

                    if not callable(name) or not isinstance(name, basestring):
                        raise Exception("name is expected to be a str or a callable. got %s" % type(name).__name__)

                    gc = group_chat.GroupChat(name, eval(self.icon, globals), eval(self.key, globals), transient=self.transient)
        ...
        def lint(self):
            name = _lint.eval(self.name)
            if name is not _lint.cant_evaluate:
                if not callable(name) or not isinstance(name, basestring):
                    _lint.error("%r is not a str or a callable" % name)
    ...
    def _parse_init_phone_register(l):
        ...
        if no_gc:
            ...
            # TODO
            # if a lambda function is given
            # turn in into an actual function
            # and use it as name
            # 
            # lambdas can't be saved, but regular functions can.
            # lots of time saved
            name = ll.require(ll.simple_expression)
