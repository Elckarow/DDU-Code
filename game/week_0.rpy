label week_0:
    ...
    window hide None
    play sound glitch
    show expression Transform(TearSurface(), xysize=(1280, 720)) as glitch
    pause 0.05
    stop music
    hide monika_scare
    pause 0.63
    ...
    python hide:
        ...
        else: # if we haven't found anything, set it to "Player"
            currentuser = __("Player")

        voice(__("prologue_32.mp3"))
        renpy.say(m, __("%s, help! Please, don’t let her take-{w=1.0}{nw}") % currentuser)
    ...
    $ console.show()
    $ console.input("undercurrents")
    $ console.output(_("Welcome to Hell."), 2.2)

    show white onlayer screens:
        alpha 0.0
        pause 0.4
        easeout_quint 4.0 alpha 1.0
