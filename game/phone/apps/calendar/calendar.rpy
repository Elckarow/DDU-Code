init -100 python in phone.calendar:
    ...
    class Calendar(calendar.Calendar):
        ...
        def is_day_passed(self, day):
            if not 0 < day <= self.length(): return False
            date = system.get_date()
            return (self.year, self.month, day) < (date.year, date.month, date.day)
        
        def length(self, offsets=False):
            if offsets: return len(self.__days)
            return self.length(True) - self.end_offset - self.start_offset
        ...
        @property
        def rows(self):
            return self.length(True) // 7
        
        def __getitem__(self, i):
            return self.__days[self.start_offset:self.length(True) - self.end_offset][i - 1]
...
screen phone_calendars():
    ...
    use _phone():
                    ...
                    if entry is None:
                        ...
                        button:
                            ...
                            if calendar.is_day_passed(entry.day):
                                background CircleDisplayable(color="#9e9e9e")
                            else:
                                hover_background CircleDisplayable(color="#e0e0e0")
                                selected_background CircleDisplayable(color="#e0e0e0")
                            ...
                            if entry.description is not None:
                                frame style "empty" align (1.0, 0.0) xysize (16, 16):
                                    background CircleDisplayable(color="#8f8f8fff")
        ...
        vbox style_prefix "phone_calendar_notes" yalign 1.0:
            ...
            frame at RoundedCorners((0, 25, 0, 25)):
