init -100 python in phone.application:
    ...
    from store import (
        Gradient, RoundedCorners, Transform, At,
    ...
    def IconBackground(d, **kwargs):
        return At(
            d,
            Transform(xysize=(gui.phone_application_icon_size, gui.phone_application_icon_size), **kwargs),
            RoundedCorners(gui.phone_application_rounded_corners_radius)
        )
            ...
            vbox:
                hbox xalign 0.5 spacing 13:
                    for i in range(max_page + 1):
                        circle color "#4e4e4e" xysize (10, 10):
                            at transform:
